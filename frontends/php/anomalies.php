...
$page['title'] = _('Anomalies analyzer');

//error ("Just testing");
$page['file'] = 'anomalies.php';
//$page['file'] = 'latest.php';
$page['type'] = detect_page_type(PAGE_TYPE_HTML);
$page['scripts'] = ['multiselect.js', 'layout.mode.js','class.calendar.js'];
...
$fields = [
	...
	'from' =>				[T_ZBX_ABS_TIME, O_OPT, null,	null,		null],
	'to' =>					[T_ZBX_ABS_TIME, O_OPT, null,	null,		null],
	...
	'anomalies_calc' =>		[T_ZBX_STR, O_OPT, P_SYS,	null,		null],
	// sort and sortorder
	'sort' =>				[T_ZBX_STR, O_OPT, P_SYS, IN('"host","lastclock","name"'),				null],
	'sortorder' =>			[T_ZBX_STR, O_OPT, P_SYS, IN('"'.ZBX_SORT_DOWN.'","'.ZBX_SORT_UP.'"'),	null],
	'periodicity' =>		[T_ZBX_INT, O_OPT, null,	null,		null],
	'duration' =>			[T_ZBX_INT, O_OPT, null,	null,		null],
	'method' =>				[T_ZBX_INT, O_OPT, null,	null,		null],
...
if (hasRequest('filter_set')) {
	CProfile::update('web.anomalies.filter.select', getRequest('select', ''), PROFILE_TYPE_STR);
	CProfile::update('web.anomalies.filter.periodicity', getRequest('periodicity', 86400), PROFILE_TYPE_INT);
	CProfile::update('web.anomalies.filter.duration', getRequest('duration', 7), PROFILE_TYPE_INT);
	CProfile::update('web.anomalies.filter.method', getRequest('method', 1), PROFILE_TYPE_INT);
	CProfile::update('web.anomalies.filter.from', getRequest('from', ""), PROFILE_TYPE_STR);
	CProfile::update('web.anomalies.filter.to', getRequest('to', ""), PROFILE_TYPE_STR);
	CProfile::update('web.anomalies.filter.application', getRequest('application', ''), PROFILE_TYPE_STR);
	CProfile::updateArray('web.anomalies.filter.groupids', getRequest('groupids', []), PROFILE_TYPE_STR);
	CProfile::updateArray('web.anomalies.filter.hostids', getRequest('hostids', []), PROFILE_TYPE_STR);
}
elseif (hasRequest('filter_rst')) {
	DBStart();
	CProfile::delete('web.anomalies.filter.select');
	CProfile::delete('web.anomalies.filter.show_without_data');
	CProfile::delete('web.anomalies.filter.periodicity');
	CProfile::delete('web.anomalies.filter.duration');
	CProfile::delete('web.anomalies.filter.method');
	CProfile::delete('web.anomalies.filter.from');
	CProfile::delete('web.anomalies.filter.to');
	CProfile::delete('web.anomalies.filter.application');
	CProfile::deleteIdx('web.anomalies.filter.groupids');
	CProfile::deleteIdx('web.anomalies.filter.hostids');
	DBend();
}

$filter = [
	'select' => CProfile::get('web.anomalies.filter.select', ''),
	'showDetails' => 0, 
	'application' => CProfile::get('web.anomalies.filter.application', ''),
	'groupids' => CProfile::getArray('web.anomalies.filter.groupids'),
	'hostids' => CProfile::getArray('web.anomalies.filter.hostids'),
	'periodicity' => CProfile::get('web.anomalies.filter.periodicity'),
	'duration' => CProfile::get('web.anomalies.filter.duration'),
	'method' => CProfile::get('web.anomalies.filter.method'),
	'from' => CProfile::get('web.anomalies.filter.from'),
	'to' => CProfile::get('web.anomalies.filter.to'),
...
if (hasRequest('anomalies_calc')) {
	error("Calculating anomalies from ".$filter['from']." to ".$filter['to']." total ".sizeof($items)." itmes");
	//var_dump($items);
	

	$anomaly_items = API::Item()->get([
		'hostids' => array_keys($hosts),
...
if (in_array($page['web_layout_mode'], [ZBX_LAYOUT_NORMAL, ZBX_LAYOUT_FULLSCREEN])) {
	// Filter
	$widget->addItem((new CFilter(new CUrl('anomalies.php')))
		->setProfile('web.anomalies.filter')
		->setActiveTab(CProfile::get('web.anomalies.filter.active', 1))
		->addFilterTab(_('Filter'), 
		[
			(new CFormList())
			->addRow((new CLabel(_('From'), 'from'))->setAsteriskMark(),
			(new CDateSelector('from', $filter['from']))
				->setDateFormat(ZBX_DATE_TIME)
				->setPlaceholder(_('YYYY-MM-DD hh:mm'))
				->setAriaRequired())
			->addRow((new CLabel(_('To'), 'to'))->setAsteriskMark(),
				(new CDateSelector('to', $filter['to']))
					->setDateFormat(ZBX_DATE_TIME)
					->setPlaceholder(_('YYYY-MM-DD hh:mm'))
					->setAriaRequired()),
			...
			(new CFormList())
						->addRow(_('Periodicity'),
						new CComboBox('periodicity', $filter['periodicity'], null, [
							3600 => _('1h'),
							7200 => _('2h'),
							10800 => _('3h'),
							14400 => _('4h'),
							86400 => _('1d')]))
				->addRow(_('Duration'),
						new CComboBox('duration', $filter['duration'], null, [
							3 => _('3x'),
							4 => _('4x'),
							5 => _('5x'),
							7 => _('7x'),
							10 => _('10x')]))
				->addRow(_('Method'),
					new CComboBox('method', $filter['method'], null, [
							1 => _('baseline AVG'),
							2 => _('baseline AVG DIFF (speed of change)')]))
							
				
				
			])) -> addItem( $items ? (new CRedirectButton(_('Look for anomalies in the items below'),
				"anomalies.php?anomalies_calc=1" ))
		//	->addClass(ZBX_STYLE_BTN_ALT)
		->addClass(ZBX_STYLE_RED_BG)
		->addClass(HALIGN_RIGHT)
